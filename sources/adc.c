...
#define MAX_STEP_TEMPERATURA 0.7
#define QUANTIDADE_MAXIMA_TEMPERATURAS 5

/* globals */
float fLeituraTemperatura;
float fTemperaturaAmostrada;
float fOldLeituraTemperatura_ADC = 0;
float fDiffTemperatura_ADC;
float fAmostrasTemperatura[10];
int iQuantidadeAmostras = 0;
float fSomaAmostras;
float fOffsetTempDegraus = 0;
...
void adc_initADCModule(void)
{
   ...
   */
   ...
   /* Inicializar fila de 5 elementos para obter a media das temperaturas */
	inicializarFilaTam5();
}
...
{
	unsigned int uiMSB, uiLSB;
	uiMSB = (ADC0_RA & 0xFF00)>>8;
	uiLSB = ADC0_RA & 0x00FF;

	float x1, x2, passo, resultado;
	x1 = tabela_temp[uiMSB]; // primeiro ponto
	x2 = tabela_temp[uiMSB+1]; // segundo ponto
	passo = (x1-x2)/256; //
	resultado = x1 + uiLSB*passo;

    return resultado; // return the register value that keeps the result of convertion
}
...
void armazenarValorAdc(void){
	////////////////////////////fTemperaturaAmostrada = adc_getConvertionValue(); /* Armazena o valor do ADC apos finalizar leitura */
	//fLeituraTemperatura = adc_getConvertionValue(); /* Armazena o valor do ADC apos finalizar leitura */
	if (TRUE == (int)adc_isAdcDone()){

		fTemperaturaAmostrada = adc_getConvertionValue(); /* Armazena o valor do ADC apos finalizar leitura */
		//fLeituraTemperatura = adc_getConvertionValue(); /* Armazena o valor do ADC apos finalizar leitura */

		fDiffTemperatura_ADC = fTemperaturaAmostrada - fOldLeituraTemperatura_ADC; /* Calcula a diferenca na temperatura lida */
		//fDiffTemperatura_ADC = fLeituraTemperatura - fOldLeituraTemperatura_ADC; /* Calcula a diferenca na temperatura lida */

		if ((0.7 < fDiffTemperatura_ADC) || (-0.7> fDiffTemperatura_ADC)){
			if ((-20 < fDiffTemperatura_ADC) && (20 > fDiffTemperatura_ADC)){
				fOffsetTempDegraus -= fDiffTemperatura_ADC;
			}
		}
	}

	/* A temperatura obtida eh a media das 10 temperaturas lidas do adc */
	adicionarElementoFilaTam5(fTemperaturaAmostrada+fOffsetTempDegraus);
	fLeituraTemperatura = obterMediaElementosTam5();

	fOldLeituraTemperatura_ADC = fTemperaturaAmostrada;
	//fOldLeituraTemperatura_ADC = fLeituraTemperatura;
}
